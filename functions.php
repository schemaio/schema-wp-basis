<?php
/**
 * Schema / Basis functions and definitions
 *
 * @package WordPress
 * @subpackage schema-wp-basis
 */
...
/**
 * Load a custom template i.e. products/cart/checkout/etc
 * Default to 404 when not found
 */
function schema_load_custom_template($wp_query = null) {

    $uri = preg_replace('/\?.*/', '', $_SERVER['REQUEST_URI']);
    $args = explode('/', trim($uri, '/.'));
    $section = array_shift($args);

    // Handle case where WP fails to load content-page.php
    // This is a bug that started occuring without a known cause
    // This condition handles it by loading content-page manually
    // When $wp_query return an post object
    if ($wp_query && $wp_query->get_queried_object()) {
        $section = 'page';
    }

    $dir_path = dirname(__FILE__);
    $template_path = $dir_path.'/'.urlencode($section).'.php';
...
/**
 * Render 404 template when a page was not found
 */
function schema_404() {

    header("HTTP/1.0 404 Not Found");
    include(dirname(__FILE__).'/_404.php');
}
...
function schema_get_cart($create_if_none_found = null) {
    ...
    if (!isset($session['cart_id']) && $create_if_none_found) {
        ...
        if (!$cart && $create_if_none_found) {
            $cart = schema_post('/carts', array('id' => $cart['id']));
            $session = schema_put_session(array('cart_id' => $cart['id']));
        }
    }
